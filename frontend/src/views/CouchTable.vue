<template>
  <v-responsive>
    <v-card border class="v-sheet" elevation="15">
      ...
      <v-card-text>
        <n-space justify="space-between">
          <v-btn class="text-caption mb-1" variant="outlined" @click="fetchData" :loading="button_update">
            Обновить таблицу
          </v-btn>
          <v-spacer/>
      ...
      <v-dialog v-model="dialog" max-width="600px">
        <v-card border rounded>
          ...
          <v-card-text>
            <v-sheet border>
              <v-text-field v-model="new_item.fullname" :rules="[rules.requiredField, rules.isString, rules.minLength]"
                            label="Фамилия Имя"
                            variant="underlined"/>
              ...
              <v-row>
                <v-col v-for="(visit_time, index) in timeItems" :key="index" :cols="height === 220 ? 6 : 4">
                  <v-checkbox
